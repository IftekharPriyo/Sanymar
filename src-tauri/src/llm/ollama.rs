...
use super::{
    prompt::build_prompt, ScriptCandidate, ScriptGenerator, ScriptGeneratorError,
    ScriptOutputError, ScriptRequest,
};
    ...
    async fn generate_inner(
        ...
        for attempt in 0..MAX_OUTPUT_ATTEMPTS {
            let response = self
                ...
                .await?;
            match parse_and_validate_output(request, &prompt.verified_fact_ids, &response) {
                Ok(output) => {
...
fn corrective_instruction(error: &ScriptOutputError, maximum_words: u16) -> String {
...
fn parse_and_validate_output(
    ...
    let mut output: StructuredOutput = serde_json::from_str(&response.message.content)
        .map_err(|_| ScriptOutputError::JsonContract)?;
...
fn map_status(status: StatusCode) -> ScriptGeneratorError {
    if status == StatusCode::NOT_FOUND {
        ScriptGeneratorError::ModelUnavailable
    } else {
        ScriptGeneratorError::Unavailable
    }
}
...
mod tests {
    ...
    fn normalizes_generated_dialogue_before_local_validation() {
        ...
        let output = parse_and_validate_output(&script_request, &HashSet::new(), &response)
