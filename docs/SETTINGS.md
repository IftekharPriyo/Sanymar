...
- `ttsVoiceId`: zero-based speaker ID reported by the selected Kokoro package. The normal UI presents this as **Voice actor** with one-based labels such as `Voice 1`; the health check rejects IDs outside the model's speaker count.
